  ...
  return (
    <TutorialLayout title="Gitを使ってみる〜Gitの概念〜">
      ...
      <p>
        ...
        <br />
        これを受けて、代替案が必要だと感じた、Linuxカーネルの開発のトップの人として知られているLinus Torvalds氏が1日で作り上げたものが<code>Git</code>です。
      </p>
      <h2>
      ...
      <p>
        ...
        <br />
        <br />
        <span className={Style.caution}>動かないプログラムを作成することがなるべく避けるべきことなのはバージョン管理ができる前も後も同じです。</span>
      </p>
      <h3>
        同時に複数の人とプログラムを書きたい
      </h3>
      ...
      <p>
        具体的には、それぞれが同じファイルでも、別々の場所を変更すると、変更したものが<code>patch</code>と呼ばれるものに変換され、差分を管理し続ける構造をGitは採用しています。
        <br />
        これによって、複数人がプログラムを変更しても、特に問題なくそれぞれの変更を融合することができる仕組みが出来上がりました。
        <br />
        <span className={Style.caution}>すべての変更がうまく融合されるわけではありません。特に、同じファイルの同じ行に対する変更は基本的にはGitだけでは解決できないため、手動で修正する必要があります。</span>
      </p>
      ...
      <p>
        ...
        <br />
        Gitでは、プロジェクトのフォルダ内に<code>.git</code>という隠しフォルダが作られ、そこにすべての情報が保存されます。
      </p>
      ...
      <p>
        一般的なGitの作業の流れは以下のようになります：
      </p>
      <ol>
        <li>ファイルを編集する</li>
        <li>変更したファイルをステージングエリアに追加する（<code>git add</code>）</li>
        <li>ステージングエリアの内容をコミットする（<code>git commit</code>）</li>
        <li>必要に応じてリモートリポジトリにプッシュする（<code>git push</code>）</li>
      </ol>
      ...
      <ul>
        <li><strong>変更履歴の追跡：</strong>いつ、誰が、何を変更したかが明確にわかる</li>
        <li><strong>安全な実験：</strong>ブランチを使って新機能を安全に試せる</li>
        <li><strong>チーム開発：</strong>複数人で同時に作業できる</li>
        <li><strong>バックアップ：</strong>リモートリポジトリに保存することで、ローカルのデータが失われても復旧できる</li>
        <li><strong>共有機能：</strong>書いたことを簡単に共有できる。</li>
      </ul>
