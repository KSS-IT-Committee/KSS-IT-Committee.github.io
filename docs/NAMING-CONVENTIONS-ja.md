...
- できるだけ,ページコンポーネントに近い場所に置くこと.
- 関係のない複数のページコンポーネントには読み込ませないこと.
...
できれば,それぞれアルファベット順にすること.

## エクスポート

コンポーネント名の誤字を防ぐために,コンポーネント・関数・型は名前付きエクスポートを基本とする.

`default export`はページコンポーネントのみ許可する.
...
- 意味のない省略をしないこと.\
  (usr, cfg, mgr)
- 似た意味の単語を複数使わない,わかりやすく分けること.\
  (UserCard と UserItem が混在するなど.)

### 略語

一般的に広く認知されている略語は使用可とする.
